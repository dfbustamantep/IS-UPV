...
public class VisualizarViajesAceptadosGUI extends JFrame {
	 ...
	 * Create the frame.
	 */
	public VisualizarViajesAceptadosGUI() {
	...
	public VisualizarViajesAceptadosGUI(Driver conductor) {
		setTitle("Visualizar viajes aceptados");
		setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
		setBounds(100, 100, 558, 351);
		contentPane = new JPanel();
	...
	public void llenarTabla(Driver conductor){
		try {
			...
			viajesAceptados = facade.get_database().getRidesAceptadosbyDriver(conductor);

			lblNombre.setText(conductor.getName());

			model.addRow(new Object[]{"Viajero", "Origen", "Destino", "Plazas Solicitadas", "Fehca", "Precio"});

			for (RideSolicitado solicitud : viajesAceptados) {
