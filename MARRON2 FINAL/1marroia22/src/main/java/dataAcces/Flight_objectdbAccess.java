...
public class Flight_objectdbAccess {
	...
	public List<ConcreteFlight> getAllConcreteFlight() {
		TypedQuery<ConcreteFlight> query = db.createQuery("SELECT cf FROM ConcreteFlight cf ORDER BY cf.flight", ConcreteFlight.class);
		List<ConcreteFlight> flights = query.getResultList();
		//for (ConcreteFlight cf : flights) System.out.println(cf);
		return flights;
	}
